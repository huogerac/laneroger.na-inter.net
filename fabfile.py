...
def staging():
    ...
    env.hosts = ["laneroger.na-inter.net", ]
    env.server_url = 'laneroger.na-inter.net'
...
def production():
    ...
    env.dev_mode = False
    ## UPDATE HERE: Enter below the staging server DOMAIN
    env.hosts = ["laneroger.yourdomain.com", ]
    env.server_url = 'laneroger.yourdomain.com'
    ## UPDATE HERE: Enter below a valid user that has SSH access to the above server
    env.user = 'user'
...
def _install_project_dependencies(projectdir):
    ...
    with cd(projectdir):
        with prefix('source ../../bin/activate'):
            ...
            run('./manage.py bower install --settings=%s.settings.%s' % (PROJECT_NAME, env.environment))
